	...
	export default {
		...
		onLaunch: function() {
			...
			*/
			...
			// 	data: req,
			// 	success: (result) => {
			// 		if (result.statusCode === 200 && result.data.status === 200 && result.data.data
			// 			.status) {
		...
		methods: {
			...mapMutations(['logout', 'getPlantformInfo', 'forceUserInfo', 'getUserInfo', 'showCartNum',
				'reloadUserResource', 'setMsgNum', 'setClientInfo', 'updatePushMessage'
			]),
			...
			getMiniInfo() {
				...
				uni.request({
					method: 'GET',
					data: '',
					url: that.Config.URL.config.miniConfig,
					success: function(data, status) {
						...
						if (data.statusCode == 200) {
							...
							console.info('----result---mini----', result);
							if (result) {

								let wxMiniId = result.wxMiniId;
								uni.setStorage({
									key: 'wxMiniId',
									data: wxMiniId,
								});

								let wxOriginId = result.wxOriginId;
								uni.setStorage({
									key: 'wxOriginId',
									data: wxOriginId,
								});

								let wxMiniPath = result.path;
								uni.setStorage({
									key: 'wxMiniPath',
									data: wxMiniPath,
								});
			...
			startLoadRequest() {
				...
				// var app_platform = uni.getStorageSync('app_platform');
				var app_version = uni.getStorageSync('app_version');
				...
				if (app_platform == 'ios') {
					...
					uni.request({
						method: 'GET',
						data: params,
						url: 'https://shop.zcskjy.com/zc-config/v1/switch/query?',
						success: function(data, status) {
							console.log('--data---1-', data, data.data.code);
							...
							if (data.statusCode == 200) {
								...
								if (result == true) {
									...
									console.log('----app_already------', app_already);
									if (app_already == true) {
										uni.setStorage({
											key: 'login_state',
											data: false,
									...
									var login_state = uni.getStorageSync('login_state');
									console.log('----login_state--01---', login_state);
			...
			*/
			...
			initJauth() {
				...
				self.jv.init({
					...
					(result) => {
						console.info('jauth init=', JSON.stringify(result));
					},
				);
				self.jv.isInitSuccess((result) => {
					console.info('jauth isInitSuccess=', JSON.stringify(result));
					if (result.enable) {
						self.jv.preLogin(5000, (result) => {
							console.info('jauth preLogin=', JSON.stringify(result));
						});
					}
				});
				// #endif
			...
			handleUniPushMsg() {
				...
				plus.push.getClientInfoAsync(
					function(info) {
						if (info.clientid) {
							//获取到clientid
							_self.setClientInfo(info);
							// _self.$store.commit('clientInfo', info);
				...
				const _handlePush = function(message) {
					// TODO
					console.log('handleUniPushMsg', message);
				};
				const _receivePush = function(message) {
					console.log('_receivePush:', message);
					_self.updatePushMessage(message);

					// // 获取推送的信息  判断是不是ios
					// if (uni.getSystemInfoSync().platform == 'ios') {
					//   let { title, content, payload } = message;
					//   plus.push.createMessage(content, title, {
					//     title: title,
					//     payload: payload,
					//     // type: payload.type,
					//     icon: 'https://qnsp.zcskjy.com/zc_images/WXshare.jpg',
					//   });
					// }
				};
				// 进入到app中某个页面
				const _clickPush = function(message) {
					console.info('_clickPush:', message);
					_self.updatePushMessage(message);
					...
					uni.switchTab({
						url: '/pages/index/index?payloadID=' + JSON.parse(payload).id,
					});
				};

				plus.push.addEventListener('click', _clickPush);
				plus.push.addEventListener('receive', function(message) {
					console.log('addEventListener-_receivePush:', message);
					_self.updatePushMessage(message);
					// plus.nativeUI.toast('push receive');
					_receivePush(message);
				});
				// #endif
			},
			...
			cVer(code) {
				var that = this;
				uni.request({
					url: that.Config.URL.config.getLeastVersion,
					data: {
						'currentVersion': code
					},
					success: (res) => {
						console.info('----info.success', res);
						const {
							code,
							result
						} = res.data
						if (code == '0' && result.needUpdate) {
							that.dlF(result.downloadUrl)
						}
					},
					fail: (err) => {
						console.info('cVer err=' + err);
					}
				});
			},

			dlF(data) {
				//'https://qnsp.zcskjy.com/zc_images%2Fstatic%2F__UNI__test.wgt'
				uni.downloadFile({
					url: data,
					success: (downloadResult) => {
						console.info('dResult=', downloadResult)
			...
			getMyMsgNum() {
				...
				let lastNumber = 0;
				that.$.request({
					url: that.Config.URL.sns.getMyMsgNum,
					// data: { num: num},
					success: function(data, status, msg, code) {
						console.log("getMyMsgNum -消息数目", data)
						lastNumber = data;
					},
					fail: () => {},
					complete: () => {
						that.msgCunt(lastNumber);
					},
				});
			},
			setMyMsgNum(num) {
				let that = this;
				that.$.request({
					url: that.Config.URL.sns.setMyMsgNum,
					data: {
						num: num
					},
					success: function(data, status, msg, code) {
						console.log("setMyMsgNum-消息数目", data)
						if (status == 200) {
						...
						if (status == 200) {
							...
							if (data.num > 0) {
								if (!data.msg_row || (data.msg_row && 1 == data.msg_row.message_type)) {
									...
									console.log("发来消息， 是否接入聊天 上次未读消息，本次未读，？", lastNumber, data.num)
									if (data.num > lastNumber) {
										that.globalTipMes(data);
									}
			...
			globalTipMes(data) {
				let that = this;
				let meaasgeTip = "发来消息， 是否接入聊天 ？";
				uni.showModal({
					title: that.$.sprintf('%s', data.msg_row
						.user_other_nickname),
					content: that.$.sprintf(meaasgeTip),
					success: function(res) {
						if (res.confirm) {
							//设置所有当前用户消息为已读
							if (data.msg_row.user_other_id) {
								//领取操作
								let tmp_params = {
									user_other_id: data.msg_row
										.user_other_id,
								};
								that.$.request({
									url: that.Config.URL.user
										.msg_set_read,
									data: tmp_params,
									loading: false,
							...
							console.log('用户点击取消', data.num);
							that.setMyMsgNum(data.num);
						}
					},
				});
			},
			checkNewMsg: function() {
				...
				if (curPage) {
					...
					if (this.$.inArray(['pagesub/webpage/web', 'pagesub/uLive/subnvue/live',
							'pagesub/uLive/subnvue/push'
						], curPage.route)) {
						return;
					}
				}
